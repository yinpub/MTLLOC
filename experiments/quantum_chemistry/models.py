...
from methods import Causals_Attention_Linear

class Net(torch.nn.Module):
    def __init__(self, n_tasks, num_features=11, dim=64,loc=True):
        super().__init__()
        ...
        if self.loc==True:
            ...
            self.dim=int(dim/4)
            self.cal = Causals_Attention_Linear(n_tasks,dim,self.dim)
        else:
            self.lin1 = torch.nn.Linear(2 * dim, dim)
    ...
    def _init_task_heads(self):
        ...
        self.task_specific = torch.nn.ModuleList([getattr(self, f"head_{i}") for i in range(self.n_tasks)])
    ...
    def forward(self, data, return_representation=False):
        ...
        if self.loc==False:
            ...
            features = self.cal(features)                # output (11)   bs=120   output[0]=[120,16]                         input: [120,64]
           
            logits = torch.cat([getattr(self, f"head_{i}")(features[i]) for i in range(self.n_tasks)], dim=1)
            if return_representation:
                return logits, features
            return logits
