...
def main(path, lr, bs, device):
    ...
    model = Network(loc=True).to(device)
    ...
    val_loader = torch.utils.data.DataLoader(dataset=val_set, batch_size=bs, shuffle=False, num_workers=2)
    ...
    weight_method = WeightMethods(
        args.method, n_tasks=40, device=device, stch=args.stch,**weight_methods_parameters[args.method],
    )
